   ...
   public function getAuditingsByUser()
{
    ...
    try {
        ...
        if ($response->successful()) {
            ...
            // Filter hanya periode status "Sedang Berjalan"
            $filtered = array_filter($data['data'], function ($item) {
                return isset($item['periode']['status']) && $item['periode']['status'] === 'Sedang Berjalan';
            });

            // Ambil data pertama yang lolos filter untuk session
            $first = reset($filtered);
            $auditingId = $first['auditing_id'] ?? null;
            $status = $first['status'] ?? null;
            $unitKerjaId = $first['unit_kerja']['unit_kerja_id'] ?? null;
