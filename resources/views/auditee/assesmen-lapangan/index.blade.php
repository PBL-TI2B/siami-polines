...
@section('title', 'Asesmen Lapangan - Auditee')
...
@section('content')
    <div class="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
        <!-- Breadcrumb -->
        <x-breadcrumb :items="[['label' => 'Assesmen Lapangan', 'url' => route('auditee.assesmen-lapangan.index')]]" />
    ...
    <x-table id="jadwalAuditTable" :headers="[
            ...
            'Status',
            'Aksi',
        ]" :data="$auditings" :data="$auditings" :route="route('auditee.assesmen-lapangan.index')">
            ...
            @forelse ($auditings as $index => $auditing)
                <tr
                    ...
                    <x-table-row-actions :actions="[
                        [
                            ...
                            'href' => route('auditee.assesmen-lapangan.edit', $auditing['auditing_id']),
                        ],
                        [
                            'label' => 'Hapus',
                            'color' => 'red',
                ...
                {{-- <x-confirmation-modal id="delete-jadwal-modal-{{ $auditing->auditing_id }}" title="Konfirmasi Hapus Jadwal"
                    :action="route('auditee.assesmen-lapangan.destroy', $auditing->auditing_id)" method="DELETE" type="delete" formClass="delete-modal-form" :warningMessage="'Menghapus jadwal ini akan menghapus seluruh data audit yang terkait.'" /> --}}
            @empty
                <tr>
