...
@section('content')
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div id="periode-container" class="mb-6 rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                {{-- Informasi Nama dan Tanggal --}}
                <div class="flex items-center gap-x-3">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-sky-100 text-sky-600 dark:bg-sky-800 dark:text-sky-300">
                ...
                <div class="mt-3 sm:mt-0">
                    <span id="periode-status-badge" class="inline-flex items-center gap-x-1.5 rounded-full px-3 py-1 text-xs font-medium">
                        Memuat status...
                    </span>
                </div>
            </div>
    ...
    document.addEventListener("DOMContentLoaded", async function() {
        ...
        try {
            ...
            // Filter data untuk mendapatkan audit dengan periode yang sedang berjalan
            const now = new Date();
            now.setHours(0, 0, 0, 0);
            const activeAudits = result.data.filter(item => {
                if (!item.periode?.tanggal_mulai || !item.periode?.tanggal_berakhir) {
                    return false;
                }
                const startDate = new Date(item.periode.tanggal_mulai);
            ...
            if (activeAudits.length === 0) {
                ...
                periodeDateRangeElem.textContent = 'Saat ini tidak ada jadwal audit yang sedang berlangsung.';
                periodeStatusBadgeElem.textContent = 'Tidak Aktif';
                periodeStatusBadgeElem.className = 'inline-flex items-center gap-x-1.5 rounded-full px-3 py-1 text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300';
                tableBody.innerHTML = `<tr><td colspan="9" class="py-6 text-center text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700">Tidak ada data audit untuk periode yang sedang berjalan.</td></tr>`;
                return;
            }
            ...
            periodeDateRangeElem.textContent = `${formatDate(activePeriode.tanggal_mulai)} - ${formatDate(activePeriode.tanggal_berakhir)}`;
            periodeStatusBadgeElem.textContent = 'Sedang Berjalan';
            periodeStatusBadgeElem.className = 'inline-flex items-center gap-x-1.5 rounded-full px-3 py-1 text-xs font-medium bg-green-100 text-green-800 dark:bg-green-800/30 dark:text-green-400';
            ...
            periodeStatusBadgeElem.className = 'inline-flex items-center gap-x-1.5 rounded-full px-3 py-1 text-xs font-medium bg-red-100 text-red-800 dark:bg-red-800/30 dark:text-red-400';
