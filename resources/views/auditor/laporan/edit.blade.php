                    ...
                    <div>
                        ...
                        <select name="standar_id" id="standar_id" class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-gray-200 sm:text-sm" required>
                            ...
                            @foreach ($standards as $standar)
                                <option value="{{ $standar['standar_id'] }}" {{ old('standar_id', $findingData['standar_id'] ?? '') == $standar['standar_id'] ? 'selected' : '' }}>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Logic to disable submit button if no kriterias or standards are available on initial load
        if (@json($kriterias).length === 0 || @json($standards).length === 0) {
            submitButton.disabled = true;
