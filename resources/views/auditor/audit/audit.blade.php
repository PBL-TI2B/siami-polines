...
@section('content')
    ...
    <div class="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
        <x-breadcrumb :items="[['label' => 'Dashboard', 'url' => route('auditor.dashboard.index')], ['label' => 'Audit']]" />
        ...
        @if ($periodeStatus === 'Berakhir')
            <div
                class="mb-6 flex flex-row gap-2 rounded-lg border-l-4 border-yellow-500 bg-yellow-100 p-4 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-200">
                <x-heroicon-s-exclamation-triangle class="h-6 w-6" /> <strong>Periode audit telah berakhir.</strong> Anda
                hanya dapat melihat progress audit, semua aksi telah dinonaktifkan.
            </div>
        @endif
            ...
            <div class="w-full lg:w-2/3">
                <div
                    ...
                    <ol class="relative ml-5 border-l-2 border-slate-200 dark:border-slate-700">
                        @php
                            ...
                            $disabled =
                                $periodeStatus === 'Berakhir'
                                    ? 'disabled aria-disabled=true tabindex=-1 style="pointer-events:none;opacity:0.6;"'
                                    : '';
                        @endphp

                        <li class="mb-10 ml-10">
                            <span
                                class="{{ $status >= 3 ? 'bg-green-500' : 'bg-slate-200 dark:bg-slate-600' }} absolute -left-5 flex h-10 w-10 items-center justify-center rounded-full ring-8 ring-white dark:ring-slate-800/50">
                                @if ($status >= 3)
                                ...
                                @else
                                    <svg class="h-5 w-5 text-slate-500 dark:text-slate-300"
                            ...
                            <h4 class="mb-1 flex items-center text-lg font-semibold text-slate-900 dark:text-white">
                                Jadwalkan Assesmen Lapangan</h4>
                            <p class="text-sm text-slate-500 dark:text-slate-400">Atur tanggal dan waktu untuk pelaksanaan
                                asesmen lapangan dengan auditee.</p>
                            @if ($status == 2)
                                <a href="{{ route('auditor.assesmen-lapangan.index', ['id' => $auditing->auditing_id]) }}"
                            ...
                            @endif
                            @if ($status == 3)
                                <a href="{{ route('auditor.audit.presensi', ['auditing' => $auditing->auditing_id]) }}"
                                    class="mt-3 inline-flex items-center rounded-lg bg-sky-600 px-4 py-2 text-sm font-medium text-white hover:bg-sky-700 focus:outline-none focus:ring-4 focus:ring-sky-300 dark:focus:ring-sky-800"
                                    {!! $disabled !!}>
                                    Download Presensi
                        ...
                        <li class="mb-10 ml-10">
                            <span
                                class="{{ $status >= 4 ? 'bg-green-500' : 'bg-slate-200 dark:bg-slate-600' }} absolute -left-5 flex h-10 w-10 items-center justify-center rounded-full ring-8 ring-white dark:ring-slate-800/50">
                                @if ($status >= 4)
                                    <svg class="h-5 w-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                        stroke-width="3">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                            ...
                            <h4 class="mb-1 text-lg font-semibold text-slate-900 dark:text-white">Koreksi Respon Instrumen
                            </h4>
                            <p class="text-sm text-slate-500 dark:text-slate-400">Periksa dan berikan skor pada jawaban
                                instrumen yang telah diisi oleh auditee.</p>
                            @if ($status == 3)
                        ...
                        <li class="mb-10 ml-10">
                            <span
                                class="{{ $status >= 5 ? 'bg-green-500' : 'bg-slate-200 dark:bg-slate-600' }} absolute -left-5 flex h-10 w-10 items-center justify-center rounded-full ring-8 ring-white dark:ring-slate-800/50">
                                @if ($status >= 5)
                                    <svg class="h-5 w-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                        stroke-width="3">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                            ...
                            <p class="text-sm text-slate-500 dark:text-slate-400">Buat pertanyaan verifikasi untuk asesmen
                                lapangan dan periksa jawabannya.</p>
                            @if ($status == 4)
                                <a href="{{ route('auditor.daftar-tilik.index', ['auditingId' => $auditing->auditing_id]) }}"
                                    class="mt-3 inline-flex items-center rounded-lg bg-sky-600 px-4 py-2 text-sm font-medium text-white hover:bg-sky-700 focus:outline-none focus:ring-4 focus:ring-sky-300 dark:focus:ring-sky-800"
                                    {!! $disabled !!}>
                                    Buat Daftar Tilik
                                    <svg class="ml-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        ...
                        <li class="mb-10 ml-10">
                            <span
                                class="{{ $status >= 7 ? 'bg-green-500' : 'bg-slate-200 dark:bg-slate-600' }} absolute -left-5 flex h-10 w-10 items-center justify-center rounded-full ring-8 ring-white dark:ring-slate-800/50">
                                @if ($status >= 7)
                            ...
                            <p class="text-sm text-slate-500 dark:text-slate-400">Susun laporan berdasarkan temuan selama
                                proses audit untuk ditinjau oleh auditee.</p>
                            @if ($status == 6)
                                <a href="{{ route('auditor.laporan.index', ['auditingId' => $auditing->auditing_id]) }}"
                                    class="mt-3 inline-flex items-center rounded-lg bg-sky-600 px-4 py-2 text-sm font-medium text-white hover:bg-sky-700 focus:outline-none focus:ring-4 focus:ring-sky-300 dark:focus:ring-sky-800"
                                    {!! $disabled !!}>
                                    Buat Laporan
                                    <svg class="ml-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        ...
                        <li class="ml-10">
                            <span
                                class="{{ $status >= 10 ? 'bg-green-500' : 'bg-slate-200 dark:bg-slate-600' }} absolute -left-5 flex h-10 w-10 items-center justify-center rounded-full ring-8 ring-white dark:ring-slate-800/50">
                                @if ($status >= 10)
                                    <svg class="h-5 w-5 text-white" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor" stroke-width="3">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                                ...
                                @else
                                    <svg class="h-5 w-5 text-slate-500 dark:text-slate-300"
                                        xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                        <path
                                            d="M10 2a5 5 0 00-5 5v2a2 2 0 00-2 2v5a2 2 0 002 2h10a2 2 0 002-2v-5a2 2 0 00-2-2H7V7a3 3 0 015.905-.75 1 1 0 001.788-1.15A5.002 5.002 0 0010 2z" />
                                    </svg>
                                @endif
                            </span>
                            <h4 class="mb-1 text-lg font-semibold text-slate-900 dark:text-white">Closing Audit</h4>
                            <p class="text-sm text-slate-500 dark:text-slate-400">Selesaikan proses audit setelah semua
                                tahapan disetujui.</p>
                            @if ($status == 9)
                                <button type="button" onclick="showCloseConfirmModal()"
                                    class="mt-3 inline-flex items-center gap-2 rounded-lg bg-sky-600 px-4 py-2 text-sm font-medium text-white hover:bg-sky-700 focus:outline-none focus:ring-4 focus:ring-sky-300 dark:focus:ring-sky-800"
                                    {!! $disabled !!}>
            ...
            <div class="w-full lg:w-1/3">
                <div
                    ...
                    <div class="mb-2 h-2.5 w-full rounded-full bg-slate-200 dark:bg-slate-700">
                        <div class="bg-{{ $progressInfo['color'] }}-500 h-2.5 rounded-full"
                            style="width: {{ $progressInfo['value'] }}%"></div>
                    </div>
                    <p class="mb-6 text-right text-sm font-medium text-slate-600 dark:text-slate-400">
                        {{ $progressInfo['value'] }}% Selesai</p>

                    <div class="mt-4 border-t border-slate-200 pt-4 dark:border-slate-700">
                        <p class="text-sm text-slate-600 dark:text-slate-400">
                            Progress akan diperbarui secara otomatis setelah Anda menyelesaikan setiap tahapan. Pastikan
                            untuk mengikuti alur yang telah ditentukan.
                        </p>
                    </div>
                </div>
            </div>
    ...
    <div id="closeConfirmModal" tabindex="-1" aria-hidden="true"
        class="fixed left-0 right-0 top-0 z-50 flex hidden h-screen w-full items-center justify-center overflow-y-auto overflow-x-hidden bg-black/50 backdrop-blur-sm md:inset-0">
        <div class="relative max-h-full w-full max-w-md p-4">
            <div
                class="relative rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800">
                <div class="flex items-center justify-between rounded-t border-b p-4 md:p-5 dark:border-slate-600">
                    <h3 class="text-xl font-semibold text-slate-900 dark:text-white">
                        Konfirmasi Closing Audit
                    </h3>
                    <button type="button" onclick="hideCloseConfirmModal()"
                        class="ms-auto inline-flex h-8 w-8 items-center justify-center rounded-lg bg-transparent text-sm text-slate-400 hover:bg-slate-200 hover:text-slate-900 dark:hover:bg-slate-600 dark:hover:text-white">
                ...
                <div class="p-4 text-center md:p-5">
                    <svg class="mx-auto mb-4 h-14 w-14 text-yellow-400" aria-hidden="true"
                ...
                <div
                    class="flex items-center justify-center gap-4 rounded-b border-t border-slate-200 p-4 md:p-5 dark:border-slate-600">
                    <button id="cancelLockBtn" type="button"
                        class="rounded-lg border border-slate-200 bg-white px-5 py-2.5 text-sm font-medium text-slate-900 hover:bg-slate-100 hover:text-sky-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-slate-200 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-white dark:focus:ring-slate-700">Batal</button>
                    <button id="confirmLockBtn" type="button"
                        class="inline-flex items-center rounded-lg bg-red-600 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-4 focus:ring-red-300 dark:focus:ring-red-800">
                        Ya, Saya Yakin
                    </button>
                </div>
            </div>
                ...
                confirmLockBtn.addEventListener('click', () => {
                    ...
                    fetch(apiUpdateUrl, {
                        ...
                        .catch(error => {
                            ...
                            showResponseModal(error.message || 'Terjadi kesalahan. Silakan coba lagi.',
                                'error');
                        })
                        .finally(() => {
